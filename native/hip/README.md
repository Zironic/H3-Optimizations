...
This source-only backend adapts Comfy Kitchen's gfx12 INT8 attention for H3's
64Q x 64KV sparse route. It targets only `gfx1200` and `gfx1201` and has not
yet been run on AMD hardware.
...
The first use runs a per-device 100%-route numerical self-test. Automatic H3
sparse selection accepts the backend only after that test passes.

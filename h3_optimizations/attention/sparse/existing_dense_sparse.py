...
This backend deliberately knows nothing about the physical attention kernel. It
owns only routing and bounded K/V packing. Each packed sparse problem is handed
back to the same Comfy attention entry point that already runs dense H3 on the
device. Policy probes 64Q x 64KV first and falls back to 128Q x 128KV when the
selected dense consumer rejects the smaller logical problem.
...
def _validate_qkv(q, k, v, *, dtype=None):
    ...
    if q.dtype != expected_dtype or k.dtype != q.dtype or v.dtype != q.dtype:
        raise ExistingDenseSparseError(
        ...
        for first in range(0, entry_count, batch_limit):
            ...
            try:
                ...
                raise ExistingDenseConsumerError(
                    'existing attention rejected packed sparse batch: %s' % error
                ) from error
...
def _execute_packed_or_dense(
    ...
    *,
    ...
    try:
        ...
        _warn_runtime_fallback('packed sparse consumer', error)
        return _dense_attention(q, k, v, transformer_options)

...
def _probe_key(device, transformer_options):
    ...
    architecture = str(getattr(properties, 'gcnArchName', 'unknown')).split(':')[0]
...
def _validate_qkv(q, k, v):
    ...
    if q.dtype != torch.bfloat16 or k.dtype != q.dtype or v.dtype != q.dtype:
        raise ExistingDenseSparseError(
            'existing-dense sparse attention requires BF16 Q/K/V'
        )
